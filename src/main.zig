...
const terminal_command: [*c][]u8 = []u8{ "alacritty", null };
...
pub fn main() void {
    ...
    var screen: c_int = c.XDefaultScreen(display); //c.DefaultScreen(display);
    ...
    var sh: c_int = c.XDisplayHeight(display, screen);
    {
        ...
        // GRAB KEYS
        {
            //UPDATE NUMLOCK MASK - USELESS
            {
                var modmap: *c.XModifierKeymap = undefined;
                defer _ = c.XFreeModifiermap(modmap);

                var numlockmask: c_int = 0;
                modmap = c.XGetModifierMapping(display);
            }
            {
                var i: u32 = undefined;
                var j: u32 = undefined;
                ...
                var modifiers = [_]u32{ c.None, c.LockMask }; // TODO, see dwm
                var code = c.XKeysymToKeycode(display, c.XStringToKeysym("F4"));
                //var code_intro = c.XK_Return;
                //_ = c.XGrabKey(display, code, c.Mod1Mask, root, c.True, c.GrabModeAsync, c.GrabModeAsync);
                //_ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XK_Return), c.Mod1Mask, root // root better
                _ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XK_Return), c.Mod1Mask, root, c.False, c.GrabModeAsync, c.GrabModeAsync);
                //_ = c.XGrabKey(display, c.AnyKey, c.AnyModifier, root, c.False, c.GrabModeAsync, c.GrabModeAsync);
    ...
    // DETECT OPENED WINDOWS
    {
        ...
        while (i < num_top_level_windows) : (i = i + 1) {
            ...
            print("Window {} detected. ", .{i});
            _ = c.XGetWindowAttributes(display, window, &x_window_attrs);
            if (x_window_attrs.override_redirect != 0 or x_window_attrs.map_state != c.IsViewable) {
                print("Non visible. Ignoring.\n", .{});
                continue;
            }
...
fn OnKeyPress(e: c.XKeyEvent, display: *c.Display) void {
    ...
    if ((e.state & c.Mod1Mask != 0) and
        (e.keycode == c.XKeysymToKeycode(display, c.XK_Return)))
    {
        //    if ((e.state & c.Mod1Mask != 0) and (e.keycode == c.XKeysymToKeycode(display, c.XK_F4))) {
        print("Spawn Terminal.\n", .{});
...
//_ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XStringToKeysym("F4")), c.Mod1Mask, window, c.False, c.GrabModeAsync, c.GrabModeAsync);
//_ = c.XGrabButton(display, 1, c.Mod1Mask, window, 1, c.ButtonPressMask | c.ButtonReleaseMask | c.PointerMotionMask, c.GrabModeAsync, c.GrabModeAsync, c.None, c.None);
//_ = c.XGrabButton(display, 1, 0, window, 1, c.ButtonPressMask | c.ButtonReleaseMask | c.PointerMotionMask, c.GrabModeAsync, c.GrabModeAsync, c.None, c.None);
//_ = c.XSelectInput(display, root, c.SubstructureRedirectMask | c.SubstructureNotifyMask | c.ExposureMask | c.ButtonPressMask | c.KeyPressMask);
//_ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XStringToKeysym("F1")), c.Mod1Mask, root, c.True, c.GrabModeAsync, c.GrabModeAsync);
//   c. Kill windows with alt + f4.
//_ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XStringToKeysym("F4")), c.Mod1Mask, root, c.False, c.GrabModeAsync, c.GrabModeAsync);
//  _ = c.XGrabKey(display, c.XKeysymToKeycode(display, c.XK_F4), c.Mod1Mask, root, 0, c.GrabModeAsync, c.GrabModeAsync);
//    _ = c.XGrabButton(display, 1, c.Mod1Mask, root, 1, c.ButtonPressMask | c.ButtonReleaseMask | c.PointerMotionMask, c.GrabModeAsync, c.GrabModeAsync, 0, 0); // 0's finals son None
//  _ = c.XGrabButton(display, 3, c.Mod1Mask, root, 1, c.ButtonPressMask | c.ButtonReleaseMask | c.PointerMotionMask, c.GrabModeAsync, c.GrabModeAsync, 0, 0);
